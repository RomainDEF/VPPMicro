...
def polaire_quille(Vs, rho_eau, Lambda, phi, workbook):
    alpha = np.arctan(np.tan(Lambda)*np.cos(phi)) #alpha = angle d'incidence entre la quille et le flux d'eau
    ...
    if Vf_nds < 0.1 : #Moins de 0.1 noeud de vitesse du bateau
        ...
        CD = 0
        S = 0
        CP_repQuille = np.array([0, 0, 0]).reshape(-1, 1) #le centre de poussée n'a aucune importance
    else : # Vitesse du bateau supérieure à 0.1 noeud
        v_bateau = [0.1, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 5, 10]
        if Vf_nds in v_bateau:
            sheet = workbook[str(Vf_nds) + "kts"]  # On ouvre la feuille de calcul correspondant au vent
            ...
            CP_repQuille = np.array([XCP, 0, ZCP]).reshape(-1, 1)
    ...
    S = 0.2
    L = 0.5 * rho_eau * CL * S * Vf ** 2
    D = 0.5 * rho_eau * CD * S * Vf ** 2
    torseur_hydro_repFluide = np.concatenate((np.array([D, L, 0]).reshape(-1, 1), np.array([0, 0, 0]).reshape(-1, 1)),
                                           axis=1)
    return torseur_hydro_repFluide, CP_repQuille

def chgt_base_pt_application(CP_repQuille, O_repQuille_repBateau, Lambda, phi):
    """
    Fonction qui passe les forces hydro du repère lié à la quille à celui lié à l'avance du bateau.
    --------------------
    ENTREES :
    CP_repQuille : coordonnées du point d'application des forces véliques dans le repère lié à la voile
                  np.array de taille (3,1) au format [X_repQuille, Y_repQuille, Z_repQuille].T
    O_repQuille_repBateau : Vecteur donnant les coordonnées de l'origine du repère lié à la voile, dans le repère lié au bateau.
    ...
    --------------------
    SORTIES :
    CP_repAvBateau  : coordonnées du point d'application des forces hydro sur la quille dans le repère d'avance du bateau.
                      vecteur du même format que CP_repQuille.
    """
    ###############################################################
    # ETAPE 1 : Passage du repère lié à la quille à celui lié au bateau.
    # Définition de la matrice de passage de repVoile à repBateau
    CP_repBateau = CP_repQuille + O_repQuille_repBateau #On fait simplement une translation, pour passer le centre de poussée dans le repère bateau
    ###############################################################
...
def chgt_base_forces(Vs, Lambda, phi, torseur_hydro_repFluide):
    ...
    """
    Lquille, Dquille = np.abs(torseur_hydro_repFluide[1][0]), np.abs(torseur_hydro_repFluide[0][0])
    Lambda_phi = np.arctan(np.tan(Lambda)*np.cos(phi))
    # FX = -Lquille*np.sin(Lambda-Lambda_phi)-Dquille*np.cos(Lambda-Lambda_phi)
    # FY = np.cos(phi)*(Lquille*np.cos(Lambda-Lambda_phi)-Dquille*np.sin(Lambda-Lambda_phi))
    # FZ = np.sin(phi)*(Lquille*np.cos(Lambda-Lambda_phi)-Dquille*np.sin(Lambda-Lambda_phi))
    FX =  -Dquille
    FY = np.cos(phi) * (Lquille) * np.sign(Lambda) #Si lambda positif, la force est positive, si lambda est négatif la force est négative
    FZ = np.sin(phi) * (Lquille)
    F_repAvBateau = np.array([FX, FY, FZ]).reshape(-1,1)
    return F_repAvBateau

def chgt_base_moments(CP_repAvBateau, F_repAvBateau):
    """
    Fonction qui passe les moments associés aux forces hydro, du repère lié au vent à celui lié à l'avance du bateau.
    --------------------
    ENTREES :
...
def torseur_Fhydro_quille(Vs, Lambda, phi, rho_eau, workbook):
    """
    ...
    O_repQuille_repBateau = np.array([1.76801,0,-0.13301]).reshape(-1,1)
    torseur_hydro_repFluide, CP_repQuille = polaire_quille(Vs, rho_eau, Lambda, phi, workbook)
    CP_repAvBateau = chgt_base_pt_application(CP_repQuille, O_repQuille_repBateau, Lambda, phi)
...
if __name__ == "__main__":
    ...
    workbookQuille = load_workbook(filename=os.path.join(pathdir,"data/Polaires_Quille_data2.xlsm"), data_only=True)
    T = torseur_Fhydro_quille(Vs, Lambda, phi, rho_eau, workbookQuille)
    print(T)